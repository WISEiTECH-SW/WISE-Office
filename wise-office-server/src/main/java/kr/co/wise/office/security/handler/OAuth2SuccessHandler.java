...
public class OAuth2SuccessHandler implements AuthenticationSuccessHandler {
    ...
    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response, Authentication authentication) throws IOException, ServletException {
        ...
        String email = customOAuthUser.getName();
        String role = customOAuthUser.getAuthorities().iterator().next().getAuthority();

        log.info("로그인 성공 JWT 발급 유저 정보 : " + email + "-- role : " + role);

        // JWT 발급
        String jwtToken = JWTUtil.createJWT(email, "ROLE_" + role);

        log.info("발급된 JWTTOKEN : " + jwtToken);
