...
public class ApproveController {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "품의서 목록 조회 결과 반환",
    ...
    public ResponseEntity<List<ApproveListResponse>> getApproveList(
        ...
        return ResponseEntity.ok(approveServiceApi.getApproveList(projectId, loginUser.getName()));
    }

    @PostMapping("/{minutesId}/approves")
    ...
    public ResponseEntity<ApproveCreateResponse> createApprove(
        ...
        return ResponseEntity.ok(approveServiceApi.createApprove(projectId, loginUser.getName(), minutesId, submitDate));
    }

    @Operation(summary = "품의서 상세 조회 API",
            security = @SecurityRequirement(name = "bearerAuth"))
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "품의서 상세 조회 결과 반환",
                    content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE,
    ...
    public ResponseEntity<ApproveDetailResponse> getDetailApprove(
        ...
        return ResponseEntity.ok(approveServiceApi.getApproveDetail(projectId, loginUser.getName(), approveId));
    }
    ...
    public ResponseEntity<ApproveUpdateResponse> updateApprove(
            @Parameter(description = "프로젝트 ID") @PathVariable long projectId,
            @Parameter(hidden = true) @AuthenticationPrincipal CustomOAuthUser loginUser,
            @Parameter(description = "품의서 수정 폼") @RequestBody ApproveUpdateRequest request) {

        return ResponseEntity.ok(approveServiceApi.updateApprove(projectId, loginUser.getName(), request));
    }
